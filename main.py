...
def confirm_loan_step(message):
    ...
    if GROUP_CHAT_ID:
        ...
        try:
            target_group = int(GROUP_CHAT_ID.strip())
            bot.send_message(target_group, group_msg, parse_mode="Markdown")
            print(f"SUCCESS: Sent message to group {target_group}")
        except Exception as e:
            print(f"ERROR sending to group {GROUP_CHAT_ID}: {e}")
